...
The private signing key must never ship with the desktop app. For the MVP, the desktop app embeds the public verification key so petpacks can be validated offline after download. Key rotation can be added later with a signed key-id strategy.
